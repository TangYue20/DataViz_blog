...
1. the Color used for differentiate 'Net Importers' and 'Net Exporters' are not distinct, which cannot makes readers quickly clarify there are two different groups based on import and export comparison.

2. The image are not necessary as it does not enhance the conveyance of information.

3. the chart looks too intensive and the label are very big, which would add reading difficulties for users.


# 2. Alternative Design
The alternative design is as follows.

![](image/72.scipts.jpg){width=70%}
...
3. Dumbbell chart clearly show the difference of import and export for Singapore merchandise with one country, which also could make it possible to compare export and import vertically and horizontally.
...
![](image/4. copy table.png){width=80%}
...
![](image/5.load data.png){width=80%}

* Drag New Union to the workspace.

![](image/6.new union.png){width=80%}

* Drag 'Import' and 'Export' into Union workplace.

![](image/7.new union2.png){width=80%}
...
![](image/17.2change eu unite formular.png){width=90%}
...
![](image/18.drag raws and column.png){width=80%}


2.  we care more about Singapore merchandise trade with major countries, so we use filter to select major countries. Drag Country from Tables to Filters box > click Edit Filters.

![](image/19.2 edit country filter.png){width=80%}
...
![](image/19.3 click 9 countries.png){width=80%}
...
![](image/21. add one more trading volume.png){width=80%}

6. Next Change similarly change the second chart type to Line.

![](image/22.change to line.png){width=80%}
...
![](image/34.rename chart name.png){width=50%}
...
![](image/35.go to sheet2.png){width=60%}
...
![](image/40.2 change into circle.png){width=90%}

14. Next add reference line. Click Analytic beside Data  > Drag Constant Line to chart, then box of Reference Line shown and drag into Export Percentile. use the same way, 
...
![](image/42.3. change import quandrant.png){width=90%}
...
![](image/51. ranking of import.png){width=90%}
...
22. Next Change the Computing Using from Automation to Country. Using the same way, we create Ranking of Export and Ranking of Total Trading Volume.
 ...
 ![](image/52. edit rankingg import.png){width=90%}


23. Drag Ranking of Export and Ranking of Import into Tooltip in Marks.

![](image/53. drag ranking to tool.png){width=90%}
...
**Insight 1**: in 2020, 10 Singapore Merchandise Trade partners could be divided into two group. one group is the countries whose value of Export exceeds import, they are Hong Kong, Mainland China, Indonesia and Malaysia. The another group whose import exceeding export are EU, Japan, Korea, Thailand and Unite State.


**Insight 2:** From 2011 to 2020,Singapore merchandise trade with Hong kong always shows the biggest difference between import and export volume compared with other countries. And the smallest difference between import and export usually could be seen in Japan and Korea.


**Insight 3:** between year 2011 and 2018, the value of export exceeds import for Singapore merchandise with Malaysia, after 2019, Singapore merchandise import from Malaysia exceeds its export to Malaysia. Besides, since year 2020, Singapore merchandise export to  Unite State exceeds its import from Unite State, prior to 2020, the import and export situation for them are opposite.
